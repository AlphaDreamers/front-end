...
async function seed() {
    try {
        ...
        const parentCategories = await Promise.all(parentCategoriesData.map((cat) => prisma.category.create({
            data: {
                title: cat.title,
                icon: cat.icon,
                color: cat.color,
        ...
        await Promise.all(subcategoriesData.map((subcat) => prisma.category.create({
            data: {
                title: subcat.title,
        ...
        await Promise.all(tags.map((title) => prisma.tag.create({
            data: { title },
        })));
        ...
        await Promise.all([...new Set(skills)].map((title) => prisma.skill.create({
            data: { title },
        })));
        ...
        const badges = [
            {
                ...
                condition: "Accumulate on-time deliveries with positive reviews (4+ stars)",
                icon: "Award",
                color: "gold",
                milestones: [
        ...
        console.log("📊 Summary:");
        console.log(`   - Categories: ${parentCategories.length + subcategoriesData.length}`);
