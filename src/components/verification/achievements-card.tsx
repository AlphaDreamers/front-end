...
import { setFeaturedBadge } from "@/lib/actions/badges";
...
export function AchievementsCard({
  ...
  const handleMakeFeatured = async (achievementId: string) =>
    toast.promise(
      async () => {
        const res = await setFeaturedBadge(achievementId);
        if (res.success === false) {
          throw new Error(res.error || "Failed to set achievement as featured");
        }
      },
      {
