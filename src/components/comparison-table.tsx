...
export default function ComparisonTable() {
  ...
  return (
    <Table
      ...
      style={{
        // @ts-expect-error --table-items-count is a custom CSS variable
      ...
      <TableHeader>
        <TableRow>
          ...
          {gigs.map((gig) => (
            <TableHead
              key={gig.id}
              className="w-[calc(100%_-_150px/var(--table-items-count))] relative group h-40"
            >
              <Button
                variant="ghost"
                size="icon"
              ...
              <Image
                src={gig.image || "/placeholder.svg?height=150&width=200"}
                alt={gig.title}
                width={200}
                height={150}
