...
const FormMultiCombobox = <T extends FieldValues>({
  control,
  name,
  icon: Icon,
  label,
          ...
          <Popover>
              ...
              <FormControl>
                ...
                >
                  {field.value.length > 0
                    ? `Selected: ${field.value.length} ${label.plural.toLowerCase()}`
                    : `Select ${label.plural.toLowerCase()}`}
            ...
            <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
              <Command>
                ...
                <CommandList>
                  <CommandEmpty>
                    No {label.plural.toLowerCase()} found.
                  </CommandEmpty>
                  <CommandGroup>
                    {values.map((val) => (
                      <CommandItem
                        ...
                        onSelect={() => {
                          const newValue = field.value.includes(val.value)
                            ? field.value.filter((v: string) => v !== val.value)
