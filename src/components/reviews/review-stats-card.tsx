      ...
      <CardContent className="space-y-1.5">
        {[...Array(Object.keys(statistics.distribution).length)].map(
          (_, index) => {
            ...
            let curStarPercentage = (curStarRatings / statistics.total) * 100;

            if (Number.isNaN(curStarPercentage)) {
              curStarPercentage = 0;
            }
            return (
...
export const ReviewStatCardSkeletion = () => {
  return (
    <Card>
      <CardHeader className="flex flex-row justify-between">
        <CardTitle className="text-2xl">Customer Reviews</CardTitle>
        <div className="flex flex-col gap-1 items-end">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-light bg-gray-300 w-16 h-4 animate-pulse" />
          </div>
          <span className="text-sm text-muted-foreground bg-gray-300 w-24 h-4 animate-pulse" />
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-1.5">
        {[...Array(5)].map((_, index) => (
          <div key={index} className="flex items-center gap-8">
            <div className="flex items-center gap-2">
              <div className="h-6 w-6 bg-gray-300 rounded-full animate-pulse" />
              <span className="text-sm text-muted-foreground font-medium w-8 bg-gray-300 h-4 animate-pulse" />
            </div>
            <div className="flex items-center flex-1 gap-2 ml-2">
              <div className="flex-1 h-2 bg-gray-300 rounded-full animate-pulse" />
              <span className="text-sm text-muted-foreground font-medium w-8 bg-gray-300 h-4 animate-pulse" />
            </div>
          </div>
        ))}
