...
if (!STRIPE_WEBHOOK_SECRET) throw new Error("Stripe webhook secret is not set");
  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed":
        ...
        if (!users) {
          console.error("Users not found for order:", orderId);
          return { received: true };
        }
        ...
        await createNotification(
          ...
          `Payment received for order "${session.metadata.orderId}"`
        );
        ...
        await createNotification(
          ...
          `Payment successful for your order "${session.metadata.orderId}"`
        );
