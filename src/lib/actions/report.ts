...
export async function getOrdersReport(filters: ReportFilters): Promise<{
    ...
    try {
        ...
        if (filters.status && filters.status !== 'all' && !['WAITING_FOR_PAYMENT', 'PENDING', 'IN_PROGRESS', 'DELIVERED', 'COMPLETED', 'CANCELLED'].includes(filters.status)) {
        ...
        const summary: ReportSummary = {
            ...
            totalRevenue: orders.reduce((sum, order) => sum + (order.package?.price || 0), 0),
            statusBreakdown: orders.reduce((acc, order) => {
                acc[order.status] = (acc[order.status] || 0) + 1;
                return acc;
            }, {} as Record<string, number>),
            averageOrderValue: orders.length > 0 ? orders.reduce((sum, order) => sum + (order.package?.price || 0), 0) / orders.length : 0,
...
export async function downloadOrdersReportPDF(filters: ReportFilters): Promise<{
    ...
    try {
        ...
        const totalRevenue = orders.reduce((sum, order) => sum + (order.package?.price || 0), 0);
        doc.setFontSize(14);
        doc.text('Summary', 20, 50);
        doc.setFontSize(12);
        doc.text(`Total Orders: ${orders.length}`, 20, 60);
        doc.text(`Total Revenue: $${totalRevenue.toFixed(2)}`, 20, 70);
        doc.text(`Average Order Value: $${orders.length > 0 ? (totalRevenue / orders.length).toFixed(2) : '0.00'}`, 20, 80);
